...
public class ContainerNpcItemGiver extends Container {
   ...
   public ContainerNpcItemGiver(EntityNPCInterface npc, EntityPlayer player) {
      ...
      for(int j1 = 0; j1 < 9; ++j1) {
         this.func_75146_a(new Slot(this.role.inventory, j1, 6 + j1 * 18, 81));
      }

      for(int i1 = 0; i1 < 3; ++i1) {
         for(int l1 = 0; l1 < 9; ++l1) {
            this.func_75146_a(new Slot(player.field_71071_by, l1 + i1 * 9 + 9, 6 + l1 * 18, 107 + i1 * 18));
         }
      }

      for(int j1 = 0; j1 < 9; ++j1) {
         this.func_75146_a(new Slot(player.field_71071_by, j1, 6 + j1 * 18, 165));
      }
