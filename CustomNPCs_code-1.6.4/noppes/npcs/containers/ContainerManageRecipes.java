...
public class ContainerManageRecipes extends Container {
   ...
   public ContainerManageRecipes(EntityPlayer player, int size) {
      ...
      this.func_75146_a(new Slot(this.craftingMatrix, 0, 87, 44));

      for(int i = 0; i < size; ++i) {
         for(int j = 0; j < size; ++j) {
            this.func_75146_a(new Slot(this.craftingMatrix, i * this.width + j + 1, j * 18 + 8, i * 18 + 18));
         }
      }

      for(int i1 = 0; i1 < 3; ++i1) {
         for(int l1 = 0; l1 < 9; ++l1) {
            this.func_75146_a(new Slot(player.field_71071_by, l1 + i1 * 9 + 9, 8 + l1 * 18, 96 + i1 * 18));
         }
      }
   ...
   public void setRecipe(RecipeCarpentry recipe) {
      this.craftingMatrix.func_70299_a(0, recipe.recipeOutput);

      for(int i = 0; i < this.width; ++i) {
         for(int j = 0; j < this.width; ++j) {
            if (j >= recipe.recipeWidth) {
               this.craftingMatrix.func_70299_a(i * this.width + j + 1, (ItemStack)null);
            } else {
               this.craftingMatrix.func_70299_a(i * this.width + j + 1, recipe.getCraftingItem(i * recipe.recipeWidth + j));
            }
         }
      }
   ...
   public void saveRecipe() {
      ...
      if (nameMapping.isEmpty()) {
         ...
         this.recipe.addRecipe(this.craftingMatrix.func_70301_a(0), recipe.toArray());
      }
   }
}
